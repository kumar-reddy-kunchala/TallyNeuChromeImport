...
let StartFunc = async ({ inKeyName, inXmlPath, inColumnsArray }) => {
    let jVarLocalTallyStatus = await FromTally({ inXmlPath });
    ...
    if (jVarLocalTallyStatus.status === 200) {
        ...
        return await jVarLocalJson;
        // BuildBsTable({ inData: JSON.parse(jVarLocalJson).ENVELOPE[inKeyName], inColumnsArray });
    };
};
